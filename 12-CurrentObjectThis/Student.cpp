...
namespace sdds {
	...
	void Student::set() {
		...
		stno = U.getInt(9999, 100000, "Student Number"); //Student number 5 digits
		cin.ignore(11111, '\n');
		set(name, stno);		// Sending temp vars to be copied into our actual attributes 
	}

	Student& Student::set(const char name[], unsigned int stno) { //Set now returns a reference object
		...
		strcpy(m_name, name);
		m_stno = stno;
		return *this;				//We use the keyword this to refer to the calling object of this function
	}
	...
	*/
	Student::Student(const char* name, const unsigned int stno) {
		init();
		set(name, stno);
	}
