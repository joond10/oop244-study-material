...
namespace sdds {
	...
	char String::operator[](size_t index) const{
		...
		char ch = 255;
		if (*this) {
			ch = m_data[index % m_length]; //safeguards for no result past m_length
		}
		return ch;
	}

	String::operator const char* () const{
		return m_data; //When our object needs to be used as a const char, return the const char member
	}
